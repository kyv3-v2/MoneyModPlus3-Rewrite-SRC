...
public enum EntityUtil implements Globals
{
    ...
    private final DamageSource EXPLOSION_SOURCE;
    
    private EntityUtil() {
        this.EXPLOSION_SOURCE = new DamageSource("explosion").setDifficultyScaled().setExplosion();
    }
    ...
    public static boolean isntValid(final EntityPlayer entity, final double range) {
        return EntityUtil.mc.player.getDistance((Entity)entity) > range || entity == EntityUtil.mc.player || entity.getHealth() <= 0.0f || entity.isDead || FriendManagement.getInstance().is(entity.getName());
    }
    
    public boolean isMoving(final EntityLivingBase entity) {
        return entity.moveStrafing != 0.0f || entity.moveForward != 0.0f;
    }
    ...
    public float calculate(final double posX, final double posY, final double posZ, final EntityLivingBase entity) {
        final double v = (1.0 - entity.getDistance(posX, posY, posZ) / 12.0) * this.getBlockDensity(new Vec3d(posX, posY, posZ), entity.getEntityBoundingBox());
        return this.getBlastReduction(entity, this.getDamageMultiplied((float)((v * v + v) / 2.0 * 85.0 + 1.0)));
    }
    
    public float getBlastReduction(final EntityLivingBase entity, final float damageI) {
        float damage = damageI;
        damage = CombatRules.getDamageAfterAbsorb(damage, (float)entity.getTotalArmorValue(), (float)entity.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).getAttributeValue());
        damage *= 1.0f - MathHelper.clamp((float)EnchantmentHelper.getEnchantmentModifierDamage(entity.getArmorInventoryList(), this.EXPLOSION_SOURCE), 0.0f, 20.0f) / 25.0f;
    ...
    public float getDamageMultiplied(final float damage) {
        ...
        for (float f = 0.0f; f <= 1.0f; f += (float)d0) {
            for (float f2 = 0.0f; f2 <= 1.0f; f2 += (float)d2) {
                for (float f3 = 0.0f; f3 <= 1.0f; f3 += (float)d3) {
                    ...
                    if (this.rayTraceBlocks(new Vec3d(d6 + d4, d7, d8 + d5), vec, false, false, false) == null) {
                        ++j2;
                    }
                    ++k2;
                }
    ...
    public int getMinArmor(final EntityPlayer player) {
        int min = 100;
        for (int j = 0; j < 4; ++j) {
            final ItemStack stack = (ItemStack)player.inventory.armorInventory.get(j);
            final int dura = 100 - (int)((1.0f - (stack.getMaxDamage() - (float)stack.getItemDamage()) / stack.getMaxDamage()) * 100.0f);
            if (dura < min) {
                min = dura;
            }
        }
